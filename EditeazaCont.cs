...
{
    public partial class EditeazaCont : Form
    {
        User utilizator;
        public EditeazaCont()
        {
           ...
           Console.WriteLine( "asta e conectat:\n"+Program.userConectat.ToString());
            tbxEmail.Text = Program.userConectat.Email;
            tbxLocalitate.Text = Program.userConectat.Localitate;
            tbxNrTelefon.Text = Program.userConectat.NrTelefon;
            tbxNume1.Text = Program.userConectat.Nume;
           
            tbxParola.Text = Program.userConectat.Parola;
            tbxVarsta.Text = Program.userConectat.Varsta.ToString();
           // textBox1.Text = "Salut";

        }

        private void EditeazaCont_Load(object sender, EventArgs e)
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (tbxNume1.Text.Trim() == "")
                eroare.SetError(tbxNume1, "Introdu nume");
            else
                if (validareDoarLitere(tbxNume1.Text) == false)
                eroare.SetError(tbxNume1, "Numele trebuie sa contina doar litere!");
            else
            ...
            //    eroare.SetError(button1, "Nu poti crea contul,deja exista un cont cu aceast email!");
                ...
                try
                {
                    ...
                    List<User> listaNouaUseri = new List<User>();
                    foreach(User user in Program.listaUtilizatori)
                    {
                        if (user.Email != utilizator.Email)
                            listaNouaUseri.Add(user);
                    }
                    listaNouaUseri.Add(utilizator);
                   // Program.listaUtilizatori.Add(utilizator);
                    Program.userConectat = utilizator;
                    BinaryFormatter bf = new BinaryFormatter();
                    FileStream fs = new FileStream("utilizatori.dat", FileMode.Create, FileAccess.Write);

                    bf.Serialize(fs,listaNouaUseri);

                    fs.Close();
                    this.Hide();
                  
                    PaginaPrincipalaConectat conectat = new PaginaPrincipalaConectat();
                    conectat.Show();
                    MessageBox.Show("Contul tau a fost editat!");
                }
                catch (Exception ex)
                {
